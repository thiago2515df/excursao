...
import { Pool } from "pg";
import { InsertUser, users, proposals, InsertProposal, Proposal } from "../drizzle/schema.postgres";
...
export async function getDb() {
  if (!_db) {
    try {
      ...
      if (!connectionString) {
        console.warn("[Database] No DATABASE_URL or DATABASE_PUBLIC_URL configured");
        return null;
      }
...
// Cleanup function to close pool on shutdown
